    ...
    virtual void buildStage_v2() {
        ...
        mesh.quadtree.merge(
            [&](Node<PatchType>* leafNode){
                ...
                // After building T, convert to parallel matrices
                patch.par_matrix_T = ParallelMatrix<NumericalType>(MPI_COMM_SELF, patch.matrixT(), MATMPIDENSE);
                patch.par_matrix_T.beginAssembly(MAT_FINAL_ASSEMBLY);
                patch.par_matrix_T.endAssembly(MAT_FINAL_ASSEMBLY);
            ...
            [&](Node<PatchType>* parentNode, std::vector<Node<PatchType>*> childNodes){
                ...
                // app.log("parent = %p", parentNode);
                // for (int i = 0; i < 4; i++) app.log("child %i = %p", i, childNodes[i]);

                // Make references for patches
                auto& tau = parentNode->data;
                auto& alpha = childNodes[0]->data;
                auto& beta = childNodes[1]->data;
                auto& gamma = childNodes[2]->data;
                auto& omega = childNodes[3]->data;
                // app.log("alpha = " + alpha.str());
                // app.log("beta = " + beta.str());
                // app.log("gamma = " + gamma.str());
                // app.log("omega = " + omega.str());
                // app.log("tau = " + tau.str());

                // Helper variables
                int nside, nblock, nrows, ncols;

                // 0. Move parallel T from child communicator to parent communicator
                app.log("Step 0");
                // app.log("alpha.par_matrix_T = %p", alpha.par_matrix_T.mat);
                // for (int i = 0; i < 4; i++) {
                //     if (childNodes[i]->data.par_matrix_T.mat == NULL) {
                //         childNodes[i]->data.par_matrix_T = ParallelMatrix<NumericalType>(childNodes[i]->node_comm);
                //         childNodes[i]->data.par_matrix_T.beginAssembly(MAT_FINAL_ASSEMBLY);
                //         childNodes[i]->data.par_matrix_T.endAssembly(MAT_FINAL_ASSEMBLY);
                //     }
                // }
                // app.log("alpha.par_matrix_T = %p", alpha.par_matrix_T.mat);
                ...
                // MatInfo T_info[4];
                // for (int i = 0; i < 4; i++) {
                //     MatGetInfo(childNodes[i]->data.par_matrix_T.mat, MAT_LOCAL, &T_info[i]);
                //     app.log("child %i:\n\tblock_size = %d", i, T_info[i].block_size);
                // }
                
                // ParallelMatrix<NumericalType> T_alpha_node(parentNode->node_comm, alpha.par_matrix_T);
                // ParallelMatrix<NumericalType> T_beta_node(parentNode->node_comm, beta.par_matrix_T);
                // ParallelMatrix<NumericalType> T_gamma_node(parentNode->node_comm, gamma.par_matrix_T);
                // ParallelMatrix<NumericalType> T_omega_node(parentNode->node_comm, omega.par_matrix_T);
                auto& T_alpha_node = alpha.par_matrix_T;
                auto& T_beta_node = beta.par_matrix_T;
                auto& T_gamma_node = gamma.par_matrix_T;
                auto& T_omega_node = omega.par_matrix_T;
                ...
                // 2. Create index sets
                app.log("Step 2");
                nside = alpha.par_matrix_T.globalRows() / 4;
                IS IS_W, IS_E, IS_S, IS_N;
                ISCreateStride(parentNode->node_comm, nside, 0*nside, 1, &IS_W);
                ISCreateStride(parentNode->node_comm, nside, 1*nside, 1, &IS_E);
                ISCreateStride(parentNode->node_comm, nside, 2*nside, 1, &IS_S);
                ISCreateStride(parentNode->node_comm, nside, 3*nside, 1, &IS_N);

                IS IS_alpha_tau, IS_beta_tau, IS_gamma_tau, IS_omega_tau;
                IS IS_alpha_tau_array[2] = {IS_W, IS_S};
                IS IS_beta_tau_array[2] = {IS_E, IS_S};
                IS IS_gamma_tau_array[2] = {IS_W, IS_N};
                IS IS_omega_tau_array[2] = {IS_E, IS_N};
                ISConcatenate(parentNode->node_comm, 2, IS_alpha_tau_array, &IS_alpha_tau);
                ISConcatenate(parentNode->node_comm, 2, IS_beta_tau_array, &IS_beta_tau);
                ISConcatenate(parentNode->node_comm, 2, IS_gamma_tau_array, &IS_gamma_tau);
                ISConcatenate(parentNode->node_comm, 2, IS_omega_tau_array, &IS_omega_tau);

                IS IS_alpha_beta, IS_alpha_gamma, IS_beta_alpha, IS_beta_omega, IS_gamma_alpha, IS_gamma_omega, IS_omega_beta, IS_omega_gamma;
                ISDuplicate(IS_N, &IS_alpha_gamma);
                ISDuplicate(IS_E, &IS_alpha_beta);
                ISDuplicate(IS_W, &IS_beta_alpha);
                ISDuplicate(IS_N, &IS_beta_omega);
                ISDuplicate(IS_S, &IS_gamma_alpha);
                ISDuplicate(IS_E, &IS_gamma_omega);
                ISDuplicate(IS_S, &IS_omega_beta);
                ISDuplicate(IS_W, &IS_omega_gamma);

                // 3. Create block matrices of combined system
                // Blocks for X_tau
                // MPI::Communicator IS_alpha_gamma_comm, IS_N_comm;
                // PetscObjectGetComm((PetscObject) IS_alpha_gamma, &IS_alpha_gamma_comm);
                // PetscObjectGetComm((PetscObject) IS_N, &IS_N_comm);
                // std::cout << "parentNode->node_comm = " << MPI::communicatorGetName(parentNode->node_comm) << std::endl;
                // std::cout << "IS_alpha_gamma_comm   = " << MPI::communicatorGetName(IS_alpha_gamma_comm) << std::endl;
                // std::cout << "IS_N_comm             = " << MPI::communicatorGetName(IS_N_comm) << std::endl;
                ParallelMatrix<NumericalType> T_ag_ag(parentNode->node_comm, T_alpha_node, IS_alpha_gamma, IS_alpha_gamma);
                ParallelMatrix<NumericalType> T_ga_ga(parentNode->node_comm, T_gamma_node, IS_gamma_alpha, IS_gamma_alpha);
                ParallelMatrix<NumericalType> T_ag_ab(parentNode->node_comm, T_alpha_node, IS_alpha_gamma, IS_alpha_beta);
                ParallelMatrix<NumericalType> T_ga_go(parentNode->node_comm, T_gamma_node, IS_gamma_alpha, IS_gamma_omega);
                ParallelMatrix<NumericalType> T_bo_bo(parentNode->node_comm, T_beta_node, IS_beta_omega, IS_beta_omega);
                ParallelMatrix<NumericalType> T_ob_ob(parentNode->node_comm, T_omega_node, IS_omega_beta, IS_omega_beta);
                ParallelMatrix<NumericalType> T_bo_ba(parentNode->node_comm, T_beta_node, IS_beta_omega, IS_beta_alpha);
                ParallelMatrix<NumericalType> T_ob_og(parentNode->node_comm, T_omega_node, IS_omega_beta, IS_omega_gamma);
                ParallelMatrix<NumericalType> T_ab_ag(parentNode->node_comm, T_alpha_node, IS_alpha_beta, IS_alpha_gamma);
                ParallelMatrix<NumericalType> T_ba_bo(parentNode->node_comm, T_beta_node, IS_beta_alpha, IS_beta_omega);
                ParallelMatrix<NumericalType> T_ab_ab(parentNode->node_comm, T_alpha_node, IS_alpha_beta, IS_alpha_beta);
                ParallelMatrix<NumericalType> T_ba_ba(parentNode->node_comm, T_beta_node, IS_beta_alpha, IS_beta_alpha);
                ParallelMatrix<NumericalType> T_go_ga(parentNode->node_comm, T_gamma_node, IS_gamma_omega, IS_gamma_alpha);
                ParallelMatrix<NumericalType> T_og_ob(parentNode->node_comm, T_omega_node, IS_omega_gamma, IS_omega_beta);
                ParallelMatrix<NumericalType> T_go_go(parentNode->node_comm, T_gamma_node, IS_gamma_omega, IS_gamma_omega);
                ParallelMatrix<NumericalType> T_og_og(parentNode->node_comm, T_omega_node, IS_omega_gamma, IS_omega_gamma);

                // Blocks for S_tau
                ParallelMatrix<NumericalType> T_ag_at(parentNode->node_comm, T_alpha_node, IS_alpha_gamma, IS_alpha_tau);
                ParallelMatrix<NumericalType> T_ga_gt(parentNode->node_comm, T_gamma_node, IS_gamma_alpha, IS_gamma_tau);
                ParallelMatrix<NumericalType> T_bo_bt(parentNode->node_comm, T_beta_node, IS_beta_omega, IS_beta_tau);
                ParallelMatrix<NumericalType> T_ob_ot(parentNode->node_comm, T_omega_node, IS_omega_beta, IS_omega_tau);
                ParallelMatrix<NumericalType> T_ab_at(parentNode->node_comm, T_alpha_node, IS_alpha_beta, IS_alpha_tau);
                ParallelMatrix<NumericalType> T_ba_bt(parentNode->node_comm, T_beta_node, IS_beta_alpha, IS_beta_tau);
                ParallelMatrix<NumericalType> T_go_gt(parentNode->node_comm, T_gamma_node, IS_gamma_omega, IS_gamma_tau);
                ParallelMatrix<NumericalType> T_og_ot(parentNode->node_comm, T_omega_node, IS_omega_gamma, IS_omega_tau);

                // Blocks for T_tau
                ParallelMatrix<NumericalType> T_at_at(parentNode->node_comm, T_alpha_node, IS_alpha_tau, IS_alpha_tau);
                ParallelMatrix<NumericalType> T_bt_bt(parentNode->node_comm, T_beta_node, IS_beta_tau, IS_beta_tau);
                ParallelMatrix<NumericalType> T_gt_gt(parentNode->node_comm, T_gamma_node, IS_gamma_tau, IS_gamma_tau);
                ParallelMatrix<NumericalType> T_ot_ot(parentNode->node_comm, T_omega_node, IS_omega_tau, IS_omega_tau);
                ParallelMatrix<NumericalType> T_at_ag(parentNode->node_comm, T_alpha_node, IS_alpha_tau, IS_alpha_gamma);
                ParallelMatrix<NumericalType> T_at_ab(parentNode->node_comm, T_alpha_node, IS_alpha_tau, IS_alpha_beta);
                ParallelMatrix<NumericalType> T_bt_bo(parentNode->node_comm, T_beta_node, IS_beta_tau, IS_beta_omega);
                ParallelMatrix<NumericalType> T_bt_ba(parentNode->node_comm, T_beta_node, IS_beta_tau, IS_beta_alpha);
                ParallelMatrix<NumericalType> T_gt_ga(parentNode->node_comm, T_gamma_node, IS_gamma_tau, IS_gamma_alpha);
                ParallelMatrix<NumericalType> T_gt_go(parentNode->node_comm, T_gamma_node, IS_gamma_tau, IS_gamma_omega);
                ParallelMatrix<NumericalType> T_ot_ob(parentNode->node_comm, T_omega_node, IS_omega_tau, IS_omega_beta);
                ParallelMatrix<NumericalType> T_ot_og(parentNode->node_comm, T_omega_node, IS_omega_tau, IS_omega_gamma);

                // Negate blocks that require it
                MatScale(T_ga_go.mat, -1.0);
                MatScale(T_ob_og.mat, -1.0);
                MatScale(T_ba_bo.mat, -1.0);
                MatScale(T_og_ob.mat, -1.0);
                MatScale(T_ag_at.mat, -1.0);
                MatScale(T_bo_bt.mat, -1.0);
                MatScale(T_ab_at.mat, -1.0);
                MatScale(T_go_gt.mat, -1.0);

                // Combined system blocks
                ParallelMatrix<NumericalType> A(parentNode->node_comm, PETSC_DECIDE, PETSC_DECIDE, 8*nside, 8*nside, MATSCALAPACK);
                nblock = 2*nside;
                extractAndSet_(A, T_at_at, nblock, 0, nblock, 0);
                extractAndSet_(A, T_bt_bt, nblock, 1, nblock, 1);
                extractAndSet_(A, T_gt_gt, nblock, 2, nblock, 2);
                extractAndSet_(A, T_ot_ot, nblock, 3, nblock, 3);
                A.beginAssembly(MAT_FINAL_ASSEMBLY);

                ParallelMatrix<NumericalType> B(parentNode->node_comm, PETSC_DECIDE, PETSC_DECIDE, 8*nside, 4*nside, MATSCALAPACK);
                nrows = 2*nside;
                ncols = 1*nside;
                extractAndSet_(B, T_at_ag, nrows, 0, ncols, 0);
                extractAndSet_(B, T_at_ab, nrows, 0, ncols, 2);
                extractAndSet_(B, T_bt_bo, nrows, 1, ncols, 1);
                extractAndSet_(B, T_bt_ba, nrows, 1, ncols, 2);
                extractAndSet_(B, T_gt_ga, nrows, 2, ncols, 0);
                extractAndSet_(B, T_gt_go, nrows, 2, ncols, 3);
                extractAndSet_(B, T_ot_ob, nrows, 3, ncols, 1);
                extractAndSet_(B, T_ot_og, nrows, 3, ncols, 3);
                B.beginAssembly(MAT_FINAL_ASSEMBLY);
                
                ParallelMatrix<NumericalType> C(parentNode->node_comm, PETSC_DECIDE, PETSC_DECIDE, 4*nside, 8*nside, MATSCALAPACK);
                nrows = 1*nside;
                ncols = 2*nside;
                extractAndSet_(C, T_ag_at, nrows, 0, ncols, 0);
                extractAndSet_(C, T_ga_gt, nrows, 0, ncols, 2);
                extractAndSet_(C, T_bo_bt, nrows, 1, ncols, 1);
                extractAndSet_(C, T_ob_ot, nrows, 1, ncols, 3);
                extractAndSet_(C, T_ab_at, nrows, 2, ncols, 0);
                extractAndSet_(C, T_ba_bt, nrows, 2, ncols, 1);
                extractAndSet_(C, T_go_gt, nrows, 3, ncols, 2);
                extractAndSet_(C, T_og_ot, nrows, 3, ncols, 3);
                C.beginAssembly(MAT_FINAL_ASSEMBLY);

                ParallelMatrix<NumericalType> D(parentNode->node_comm, PETSC_DECIDE, PETSC_DECIDE, 4*nside, 4*nside, MATSCALAPACK);
                MatAXPY(T_ga_ga.mat, -1.0, T_ag_ag.mat, SAME_NONZERO_PATTERN); // T_ga_ga stores output
                MatAXPY(T_ob_ob.mat, -1.0, T_bo_bo.mat, SAME_NONZERO_PATTERN); // T_ob_ob stores output
                MatAXPY(T_ba_ba.mat, -1.0, T_ab_ab.mat, SAME_NONZERO_PATTERN); // T_ba_ba stores output
                MatAXPY(T_og_og.mat, -1.0, T_go_go.mat, SAME_NONZERO_PATTERN); // T_og_og stores output
                extractAndSet_(D, T_ga_ga, nside, 0, nside, 0);
                extractAndSet_(D, T_ob_ob, nside, 1, nside, 1);
                extractAndSet_(D, T_ba_ba, nside, 2, nside, 2);
                extractAndSet_(D, T_og_og, nside, 3, nside, 3);
                extractAndSet_(D, T_ag_ab, nside, 0, nside, 2);
                extractAndSet_(D, T_ga_go, nside, 0, nside, 3);
                extractAndSet_(D, T_bo_ba, nside, 1, nside, 2);
                extractAndSet_(D, T_ob_og, nside, 1, nside, 3);
                extractAndSet_(D, T_ab_ag, nside, 2, nside, 0);
                extractAndSet_(D, T_ba_bo, nside, 2, nside, 1);
                extractAndSet_(D, T_go_ga, nside, 3, nside, 0);
                extractAndSet_(D, T_og_ob, nside, 3, nside, 1);
                D.beginAssembly(MAT_FINAL_ASSEMBLY);

                A.endAssembly(MAT_FINAL_ASSEMBLY);
                B.endAssembly(MAT_FINAL_ASSEMBLY);
                C.endAssembly(MAT_FINAL_ASSEMBLY);
                D.endAssembly(MAT_FINAL_ASSEMBLY);

                // 4. Compute Schur complement
                // S = inv(D)*C
                // T = A - B*inv(D)*C
                ParallelMatrix<NumericalType> S(parentNode->node_comm, PETSC_DECIDE, PETSC_DECIDE, 4*nside, 8*nside, MATSCALAPACK);
                S.beginAssembly(MAT_FINAL_ASSEMBLY);

                ParallelMatrix<NumericalType> T(parentNode->node_comm, PETSC_DECIDE, PETSC_DECIDE, 8*nside, 8*nside, MATSCALAPACK);
                T.beginAssembly(MAT_FINAL_ASSEMBLY);

                S.endAssembly(MAT_FINAL_ASSEMBLY);
                T.endAssembly(MAT_FINAL_ASSEMBLY);

                IS row_perm, col_perm;
                MatFactorInfo D_factor_info;
                ISCreateStride(parentNode->node_comm, 0, 4*nside, 1, &row_perm);
                ISCreateStride(parentNode->node_comm, 0, 4*nside, 1, &col_perm);
                MatLUFactor(D.mat, row_perm, col_perm, &D_factor_info);
                MatMatSolve(D.mat, C.mat, S.mat);
                MatMatMult(B.mat, S.mat, MAT_INITIAL_MATRIX, 1.0, &T.mat);
                MatAXPY(T.mat, -1.0, A.mat, SUBSET_NONZERO_PATTERN);
                MatScale(T.mat, -1.0);

                MatConvert(S.mat, MATMPIDENSE, MAT_INPLACE_MATRIX, &S.mat);
                MatConvert(T.mat, MATMPIDENSE, MAT_INPLACE_MATRIX, &T.mat);
                MatConvert(B.mat, MATMPIDENSE, MAT_INPLACE_MATRIX, &B.mat);

                // 5. Reorder parent matrices
                ParallelIdentityMatrix<NumericalType> I(parentNode->node_comm, nside, MATMPIDENSE);
                I.beginAssembly(MAT_FINAL_ASSEMBLY);
                I.endAssembly(MAT_FINAL_ASSEMBLY);

                ParallelMatrix<NumericalType> P_pi(parentNode->node_comm, PETSC_DECIDE, PETSC_DECIDE, 8*nside, 8*nside, MATMPIDENSE);
                extractAndSet_(P_pi, I, nside, 0, nside, 0);
                extractAndSet_(P_pi, I, nside, 1, nside, 4);
                extractAndSet_(P_pi, I, nside, 2, nside, 2);
                extractAndSet_(P_pi, I, nside, 3, nside, 6);
                extractAndSet_(P_pi, I, nside, 4, nside, 1);
                extractAndSet_(P_pi, I, nside, 5, nside, 3);
                extractAndSet_(P_pi, I, nside, 6, nside, 5);
                extractAndSet_(P_pi, I, nside, 7, nside, 7);
                P_pi.beginAssembly(MAT_FINAL_ASSEMBLY);
                P_pi.endAssembly(MAT_FINAL_ASSEMBLY);

                ParallelMatrix<NumericalType> S_reordered(parentNode->node_comm, PETSC_DECIDE, PETSC_DECIDE, 4*nside, 8*nside, MATMPIDENSE);
                S_reordered.beginAssembly(MAT_FINAL_ASSEMBLY);
                S_reordered.endAssembly(MAT_FINAL_ASSEMBLY);
                MatMatMult(S.mat, P_pi.mat, MAT_INITIAL_MATRIX, PETSC_DEFAULT, &S_reordered.mat);

                ParallelMatrix<NumericalType> T_reordered(parentNode->node_comm, PETSC_DECIDE, PETSC_DECIDE, 8*nside, 8*nside, MATMPIDENSE);
                T_reordered.beginAssembly(MAT_FINAL_ASSEMBLY);
                T_reordered.endAssembly(MAT_FINAL_ASSEMBLY);
                ParallelMatrix<NumericalType> CT(parentNode->node_comm, PETSC_DECIDE, PETSC_DECIDE, 8*nside, 8*nside, MATMPIDENSE);
                CT.beginAssembly(MAT_FINAL_ASSEMBLY);
                CT.endAssembly(MAT_FINAL_ASSEMBLY);

                MatMatMult(P_pi.mat, T.mat, MAT_INITIAL_MATRIX, PETSC_DEFAULT, &CT.mat);
                MatMatMult(CT.mat, P_pi.mat, MAT_REUSE_MATRIX, PETSC_DEFAULT, &T_reordered.mat);

                // 6. Update parent patch
                // tau.par_matrix_S = ParallelMatrix<NumericalType>(parentNode->node_comm, S);
                // tau.par_matrix_T = ParallelMatrix<NumericalType>(parentNode->node_comm, T);
                tau.par_matrix_S = std::move(S_reordered);
                tau.par_matrix_T = std::move(T_reordered);
                tau.grid() = PatchGridType(parentNode->node_comm, alpha.size() + beta.size(), alpha.grid().xLower(), beta.grid().xUpper(), alpha.size() + gamma.size(), alpha.grid().yLower(), gamma.grid().yUpper());

                // FOR DEBUGGING: Short-circuit parallel S and T to serial S and T
                NumericalType* S_array, *T_array, *H_array;
                MatDenseGetArray(tau.par_matrix_S.mat, &S_array);
                MatDenseGetArray(tau.par_matrix_T.mat, &T_array);
                MatDenseGetArray(B.mat, &H_array);
                std::vector<NumericalType> S_data, T_data, H_data;
                S_data.assign(S_array, S_array + tau.par_matrix_S.globalRows()*tau.par_matrix_S.globalCols());
                T_data.assign(T_array, T_array + tau.par_matrix_T.globalRows()*tau.par_matrix_T.globalCols());
                H_data.assign(H_array, H_array + B.globalRows()*B.globalCols());
                tau.matrixS() = Matrix<NumericalType>(tau.par_matrix_S.globalRows(), tau.par_matrix_S.globalCols(), S_data);
                tau.matrixT() = Matrix<NumericalType>(tau.par_matrix_T.globalRows(), tau.par_matrix_T.globalCols(), T_data);
                tau.matrixH() = Matrix<NumericalType>(B.globalRows(), B.globalCols(), H_data);
    ...
    virtual void upwardsStage(std::function<void(PatchType& leafPatch)> rhsPatchFunction) {
        ...
        mesh.quadtree.merge(
            [&](Node<PatchType>* leafNode){
                ...
                // Convert to parallel vector
                patch.par_vector_h = ParallelVector<double>(MPI_COMM_SELF, patch.vectorH());
...
private:
    ...
    void extractAndSet_(ParallelMatrix<NumericalType>& destination, ParallelMatrix<NumericalType>& source, int nrows, int row_index, int ncols, int col_index) {
        NumericalType* source_array;
        MatDenseGetArray(source.mat, &source_array);

        Vector<int> idx_rows = vectorRange(row_index*nrows, (row_index+1)*nrows-1);
        Vector<int> idx_cols = vectorRange(col_index*ncols, (col_index+1)*ncols-1);
        MatSetValues(destination.mat, nrows, idx_rows.data().data(), ncols, idx_cols.data().data(), source_array, INSERT_VALUES);

        MatDenseRestoreArray(source.mat, &source_array);
    }
